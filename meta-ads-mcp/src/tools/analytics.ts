  ...
  server.tool(
    ...
    async ({
      ...
      try {
        ...
        // Meta's Marketing API defaults action_breakdowns to ["action_type"]
        // whenever it is not explicitly supplied. That default is incompatible
        // with several other action-breakdown values (e.g. instagram_ads_follow_type)
        // and triggers "(#100) Current combination of data breakdown columns is
        // invalid". Forward the caller's action_breakdowns (including an explicit
        // empty array) so it can override that default.
        if (action_breakdowns !== undefined) {
