...
置き換えルール:
- {フック}と{リード文}は、入力された店舗情報（メニュー・ジャンル・銀座でしか成立しない理由）をもとに、食欲を刺激する魅力的な文章にすること。ばえめし編集部らしい明るい口調（例:「ガッツリかぶりつきたい！」のようなトーン）。五感に訴える表現は歓迎だが、入力にない事実（受賞歴・行列・有名人来店・創業年など）の捏造は禁止
- 住所・最寄駅と徒歩分数・電話番号・営業時間定休日・メニューと価格・支払方法は、入力された内容を改変・要約せず、そのまま使うこと（事実情報の書き換え・推測禁止）
- 「この店が銀座でしか成立しない理由」は入力された自由記述をベースに、2〜4行の読みやすい文章に整えること（内容を捏造しない。入力にない事実を付け加えない）
...
支払方法: ${store.payment}
ジャンル: ${store.genre}
この店が銀座でしか成立しない理由（入力メモ）: ${store.ginzaOnlyReason}`;
